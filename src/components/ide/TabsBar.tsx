...
export function TabsBar({ isMobile }: { isMobile?: boolean }) {
  ...
  if (tabs.length === 0) return <div className={cn("shrink-0 border-b border-border bg-sidebar-bg", isMobile ? "h-11" : "h-9")} />;

  return (
    <LayoutGroup>
      <div className={cn("flex shrink-0 items-stretch border-b border-border bg-sidebar-bg ide-scrollbar overflow-x-auto", isMobile ? "h-11" : "h-9")}>
          ...
          {tabs.map((tab) => {
            ...
            return (
              <motion.div
                ...
                transition={isMobile ? { duration: 0.1 } : { type: "spring", stiffness: 450, damping: 35, mass: 0.8 }}
                className={cn(
              ...
              >
                <button
                  ...
                  className={cn("flex items-center gap-2 font-mono", isMobile ? "py-2 text-[13px]" : "py-1 text-[12.5px]")}
                >
                  <Icon className={cn(isMobile ? "h-4 w-4" : "h-3.5 w-3.5", color)} strokeWidth={1.6} />
                  <span className={cn("whitespace-nowrap", isMobile && "hidden sm:inline")}>{tab.label}</span>
                </button>
                <button
                ...
                {active && (
                  <motion.span
                    ...
                    transition={isMobile ? { duration: 0.1 } : { type: "spring", stiffness: 500, damping: 40 }}
                  />
                )}
              </motion.div>
            );
